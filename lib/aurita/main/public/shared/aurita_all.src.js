...
function on_hierarchy_entry_reorder(entry)
{
    ...
    cb__load_interface_silently('dispatcher','/aurita/Hierarchy/perform_reorder/' + position_values + '&hierarchy_id=' + reorder_hierarchy_id); 
}
...
//        element.innerHTML = Cuba.loading_symbol; 
    ...
    // Usage: 
    // <span onclick="Cuba.confirmable_action({ action: 'Community::Forum_Post/delete/forum_post_id=123', 
    ...
    confirmable_action: function(params) {
      ...
      Cuba.message_box = new MessageBox({ interface_url: '/aurita/App_Main/confirmation_box/message='+message }); 
  ...
  //  Element.setStyle('file_upload_indicator', { display: '' });
    ...
    after_file_upload: function() { 
      if(Cuba.waiting_for_file_upload) {
        ...
        Cuba.load({ element: 'app_main_content', action: 'Wiki::Media_Asset/show_latest' }); 
      }
    }, 
    upload_file: function(form) {
      ...
      Element.toggle(form.id + '_wrap'); 
  //  context_menu_close();  DO NOT CLOSE CONTEXT MENU! Upload will not start in case submitting form is removed 
  //                         from page before actual submit! 
