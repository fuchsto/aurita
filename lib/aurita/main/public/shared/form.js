...
Aurita.get_remote_string = function(action, response_fun) { 
  ...
  xml_conn.get_string('/aurita/'+action+'&mode=async', response_fun);
}; 
...
Aurita.submit_upload_form = function(form_id) { 
  ...
  { 
    alert('Machen Sie alle erforderlichen Angaben'); 
    return false; 
  }
...
Aurita.toggle_box = function(box_id) { 
...
Aurita.handle_feedback = function(response) 
{
  ...
  if(feedback.random_image) { 
    if($('random_image') && Aurita.last_feedback.random_image != feedback.random_image) { 
  ...
  if(feedback.recently_changed) { 
    if(!Aurita.last_feedback.recently_changed || 
       !Aurita.compare_arrays(Aurita.last_feedback.recently_changed, feedback.recently_changed)) { 
      log_debug('-- recently_changed'); 
      Aurita.load({ element: 'changed_articles_body', action: 'Wiki::Article/print_recently_changed/', nocache: true }); 
    }
  }
...
{
  Aurita.get_remote_string('Async_Feedback/get/'+Aurita.random(), Aurita.handle_feedback); 
}

// setInterval(Aurita.poll_feedback, 30000); 

Aurita.poll = function() { 
  Aurita.load_silently({ action: 'App_General.users_online_box_body', element: 'users_online_box_body' }); 
}
setInterval(Aurita.poll, 60000);


Aurita.confirmed_interface = ''; 
Aurita.unconfirmed_interface =  '';  
Aurita.message_box = undefined; 
Aurita.on_confirm_action = function() {}; 
...
Aurita.confirmable = function(params) {
  ...
  Aurita.unconfirmed_interface = interface_url; 
  if(params['onconfirm']) { 
...
Aurita.confirm_action = function() { 
  Aurita.call({ action: Aurita.unconfirmed_interface, 
                on_update: Aurita.after_confirmed_action });
  Aurita.on_confirm_action(); 
