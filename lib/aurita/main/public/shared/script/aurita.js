...
Aurita.async_form_submit = function(form_element, params) { 
  ...
  if(params) { 
    ...
    if(params.element) { 
      element = Aurita.element(params.element); 
    }
    else {
      element = Aurita.element('dispatcher');
    }

    if(element && element.id) { postVars += '&element='+element.id; }

    onload_fun = params.onload; 
    if(params.hide_on_submit) { 
      // TODO: ancestors()[2] is specialized for forms 
      // decorated using Async_Form_Decorator and might 
      // not work with others. 
      onload_fun = function() { Element.hide($(form_element).ancestors()[2]); };
    }
  }
...
Aurita.file_upload_failed = false; 
...
Aurita.update_element_silently = function(xml_conn, params) // params: element, request_method, onload
{
    var element     = params.element; 
    var onload_fun  = params.onload; 
...
Aurita.replace_element_silently = function(xml_conn, params) // params: element, request_method, onload
{
    var element     = params.element; 
    var onload_fun  = params.onload; 
...
Aurita.update_element = function(xml_conn, params) // params: element, request_method, onload
{
    var element     = params.element; 
    var onload_fun  = params.onload; 
    var target; 
    ...
    if(element) 
    {
      try { Element.setOpacity(element, 1.0); } catch(e1) { }
      ...
      {
        try { Aurita.context_menu_close(); } catch(e2) { } 
      } 
      element.innerHTML = response_html; 
    }
    ...
    if(response_script) { 
      eval(response_script.replace('\"','"')); 
    }
    
    if(Aurita.update_targets) {
      for(target in Aurita.update_targets) {
...
Aurita.before_load_url = function(params) { 
  if(params.element) { 
    try { Element.setOpacity(params.element, 0.5); } catch(e1) { } 
  }
  try { Aurita.Editor.save_all(); } catch(e2) { } 
};

/* Basic function for asynchronous requests. 
 * Usage: 
 *
 *   Aurita.load_url({ element: 'dom_id_of_target_element', 
 *                     action: 'Wiki::Article/show/article_id=123', 
 *                     method: 'GET', // default
 *                     onload: function() { alert('done!') }, 
 *                     handler_fun: optional_handler }); 
 *
 * An optional callback function looks like this: 
 *
 *   function optional_handler(response, params) { 
 *     var target_element  = params.element; 
 *     var request_method  = params.method; 
 *     var onload_callback = params.onload; 
 *     var response        = Aurita.eval_response(response); 
 *
 *     var html_resp       = response.html; 
 *     var script_resp     = response.script; 
 *
 *     // ...
 *
 ...
 *   }
 *
 * Note that the handler function has precedence over the 
 * onload callback and is responsible for calling it 
 * (or deciding not to do so, e.g. in case of an error). 
 *
 */
...
Aurita.load_url = function(params)
{
  ...
  if(params.action) { 
    ...
    if(element && element.id) { action_url += ('element=' + element.id); }
  ...
  if(params.silently) { 
    ...
    log_debug('LOAD URL'); 
    update_fun   = params.handler_fun; 
    if(!update_fun) { update_fun = Aurita.update_element; }
  }
...
Aurita.insert = function(params) { 
  var handler_fun = false;
  var container   = document.createElement('span');
  var cnt         = 0; 
  var container_id;

  if(params.before_element) { 
    cnt = 0; 
    container_id = 'before_' + params.before_element + '_';
    // Look for next available unique DOM id for this container
    while(Aurita.element(container_id + cnt)) { 
      cnt++;
    }
    container_id = container_id + cnt; 
    container.id = container_id; 
    container.addClassName('insertion_container');
    $(params.before_element).insert({ top: container }); 
    container.innerHTML = '<img class="loading_icon" src="/aurita/images/icons/loading.gif" />';
    params.element = container_id;
  } 
  else if(params.after_element) { 
    cnt = 0; 
    container_id = 'after_' + params.after_element + '_';
    // Look for next available unique DOM id for this container
    while(Aurita.element(container_id + cnt)) { 
      cnt++;
    }
    ...
    container.id = container_id; 
    container.addClassName('insertion_container');
    $(params.after_element).insert({ after: container }); 
    container.innerHTML = '<img class="loading_icon" src="/aurita/images/icons/loading.gif" />';
    params.element = container_id;
  }
  params.before_element = undefined; 
  params.after_element  = undefined; 
  params.handler_fun    = Aurita.replace_element_silently;
 ...
 * Usage: 
 *
 *   Aurita.load_widget('Aurita::Plugins::Wiki::GUI::SomeWidget', 
 *                      { ctor_params: 'for', the: 'widget' }, 
 *                      Aurita.load_widget_to('dom_id_of_element_to_append_at'));
 *
 *
 **/
Aurita.load_widget_to = function(element) { 
  return function(widget_response) { 
    var widget, widget_html, init_script; 
 ...
 *   Aurita.load_widget('Aurita::Plugins::Wiki::GUI::SomeWidget', 
 *                      { ctor_params: 'for', the: 'widget' }, 
 *                      Aurita.append_widget_to('dom_id_of_element_to_append_at'));
 *
 *
 **/
Aurita.append_widget_to = function(element) { 
  return function(widget_response) { 
    var widget, widget_element, widget_script; 
...
Aurita.get_hashvalue = function() {
  hv = document.location.hash; 
  if(hv) {
    return hv.replace('#','');
  }
  else { 
...
{
    var iframe_hashvalue = false; 

    Aurita.current_hashvalue = Aurita.get_hashvalue(); 

    if(!Aurita.current_hashvalue || Aurita.current_hashvalue.match(/(.+)?_anchor/)) { return;  } 

    if(Aurita.check_if_internet_explorer() == 1) { // IE REACT
      log_debug("IE load procedure 1");
...
//      }
        ...
        log_debug("IE load procedure 4");
      }
      log_debug("IE load procedure 5");
    }
