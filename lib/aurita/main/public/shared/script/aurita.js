...
Aurita.set_hashcode = function(code) 
{
  ...
  document.location.hash = current_hashvalue;
...
/*
  if(Aurita.current_request) { 
    if(!Aurita.current_request.bComplete) { 
      ...
      if(Aurita.current_request.req_url() == req_url) { 
// TODO: Verify / test this! 
//      Aurita.current_request.abort(); 
        log_debug("Cancelled request for " + req_url); 
      }
    }
  }
*/
...
Aurita.check_hashvalue = function() 
{
    ...
    if(Aurita.check_if_internet_explorer() == 1) { // IE REACT
      ...
      if(iframe_hashvalue && 
         ...
         iframe_hashvalue != current_hashvalue && 
         !Aurita.force_load && 
         iframe_hashvalue != '' && 
  ...
  log_debug('check_hashvalue load'); 
      ...
      log_debug('new: ' + current_hashvalue); 
