...
//    y: 120, 
  ...
  Autocomplete : function(element_id, params) { 
    ...
    $(e_id).observe('focus', function(evt) { 
      ...
      autocompleter.poll   = setInterval(function() { 
        key = $(e_id).value; 
        if (key.length > 2 && autocompleter.last_key != key) {
          autocompleter.last_key = key; 
          if (params.onupdate) { params.onupdate(); }
          Aurita.load_silently({ action: params.action + 'key=' + key, 
                                 element: params.element, 
                                 onload: params.onload });
        }
      }, 400);
    });
    $(e_id).observe('blur', function(evt) { 
      this.active = false; 
      clearInterval(poll);
