...
//    y: 120, 
  ...
  center_element_y : function(element) {
    if($(element) != null) {

      var offset_y = 0; 
      if(window.innerHeight) {
        offset_y = Math.round(document.viewport.getScrollOffsets().top + window.innerHeight/2)
      } else {
        offset_y = Math.round(document.body.scrollTop + ($$('body')[0].clientHeight)/2)
      }
  ...
  Tab_Group : { 
    ...
    init : function(tab_group_id, params) { 
      Aurita.GUI.Tab_Group.tab_groups[tab_group_id] = new Object; 
      if(params && params.active) { 
        Aurita.GUI.Tab_Group.tab_groups[tab_group_id].active = $(params.active);
      }

      $$('#'+tab_group_id+' .tab').each(function(elem, idx) { 
        if(elem == Aurita.GUI.Tab_Group.tab_groups[tab_group_id].active) { 
          elem.addClassName('selected');
        }
        elem.observe('click', function(evt) { 
          var elem   = evt.element(); 
          var active = Aurita.GUI.Tab_Group.tab_groups[tab_group_id].active;
          log_debug(elem.id + ' == ' + active.id);
          if(elem.id == active.id) { return; } 
          active.removeClassName('selected');
          elem.addClassName('selected');
          Aurita.GUI.Tab_Group.tab_groups[tab_group_id].active = elem; 
        });
      });
    }
  ...
  Form : { 
    ...
    submit_flash_upload_form : function(params) {
      var form_id   = params.form;
      var applet_id = params.swf;
      ...
      try { 
        // Mozilla and WebKit use <embed> part of flash embedding
        cb_return = $(applet_id).setParams(base_uri, values);
      } catch(e) {
        // IE uses <object> part of flash embedding
        applet_id = applet_id.replace('_target', '_container');
        cb_return = $(applet_id).setParams(base_uri, values);
      }

      if(cb_return == 'error') { return; } 

      // Remove all form elements except for applet: 
      $(form_id+'_fields').childElements().each(function(elem, idx) { 
        if(elem.id != 'flash_upload_applet_wrap') { elem.hide(); } 
      });
      $(form_id+'_buttons').hide(); 

      $(applet_id).startUpload(); 
    }
