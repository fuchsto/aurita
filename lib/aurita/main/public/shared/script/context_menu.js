...
function ContextMenu(menu_element_id, params)
{
    ...
    var open_menu = function(params) {
        ...
        m_menu.innerHTML = Cuba.loading_symbol; 
    ...
    this.set = function(params) {     
      ...
      m_autoclose = params['autoclose']; 
      if(params.targets) { Cuba.update_targets = params.targets; }
      if(params.url)     { m_interface         = params.url; }
...
//        try { init_autocomplete_username(); } catch(e) { }
        }
      }
    };
...
//      ?    m_focussed_element_id = false; 
//      ?    m_clicked_element_id = false; 
//      ?    m_active_element_id = false; 
//      ?    m_highlight_element_id = false; 
        } 
    };
    ...
    this.element_hover = function(type, args, elem_id, highlight_element_id) {
      ...
      if(false && m_opened) 
      { 
          m_other_clicked_element_id = elem_id; 
          m_other_args = args; 
          m_other_interface = '/aurita/'+context_menu_controller+'/'+type.toLowerCase()+'/'+args;
      }
      else 
      { 
    ...
    this.handle_click = function(ev, is_right_click) {
      ...
      if(ie_right_click || is_right_click) {
...
var context_menu_handle = new ContextMenu('context_menu'); 

function context_menu_click(params)
{
//  params.url = '/aurita/' + params.url;
//  context_menu_handle.set(params); 
    context_menu_handle.no_autoclose(); 
    Element.hide('context_menu'); 
    Aurita.load({ element: 'context_menu', 
                  action: params.url, 
                  onload: function() { Aurita.center_element('context_menu'); Element.show('context_menu'); } }); 
}
function context_menu_close()
{
    context_menu_handle.close(); 
    if(Cuba.message_box) { 
...
// doesn't open it itself (see cb__context_menu). 
var last_hovered = false; 
function context_menu(type, args, elem_id, highlight_element_id) 
{
  context_menu_handle.element_hover(type, args, elem_id, highlight_element_id); 
  if(last_hovered) { Element.removeClassName(last_hovered, 'context_hover'); }
  if(highlight_element_id) { 
    Element.addClassName(highlight_element_id, 'context_hover'); 
    last_hovered = highlight_element_id; 
  }
  else { 
    Element.addClassName(elem_id, 'context_hover'); 
    last_hovered = elem_id; 
  }
  window.status = type;
  context_menu_handle.enable(); 
//  Element.show('context_menu_icon'); 
}

function context_menu_opened() { 
  return context_menu_handle.is_opened(); 
}

function context_menu_out(element) 
{ 
  context_menu_handle.disable(); 
  Element.removeClassName(last_hovered, 'context_hover'); 
//  Element.hide('context_menu_icon'); 
} 
