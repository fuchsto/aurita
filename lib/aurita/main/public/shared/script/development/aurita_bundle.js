...
Aurita.update_element = function(xml_conn, element, request_method, onload_fun)
{
    ...
    log_debug('Rendering response ...'); 
    if(element) 
    {
      log_debug('Rendering response 1'); 
      try { Element.setOpacity(element, 1.0); } catch(e) { }

      log_debug('Rendering response 2'); 
      if(response_debug) { log_debug(response_debug); }

      log_debug('Rendering response 3'); 
      // When to close context menu (no error and no html response, or target element
      // is not context menu)
      log_debug('Rendering response 4'); 
      if(!response_error && (!element || !response_html))
      {
        Aurita.context_menu_close(); 
      } 
      ...
      log_debug('Rendering response 5'); 
      element.innerHTML = response_html; 
    }
    if(onload_fun) { 
      onload_fun(element); 
...
Aurita.check_hashvalue = function() 
{
  ...
  log_debug('check_hashvalue load'); 
      ...
      log_debug("loading interface for hash code "+Aurita.current_hashvalue); 
...
function ContextMenu(menu_element_id, params)
{
    ...
    var open_menu = function(params) {
        ...
        m_menu.innerHTML = Aurita.loading_symbol; 
    ...
    this.handle_click = function(ev, is_right_click) {
      ...
      if(ie_right_click || is_right_click) {
        ...
        if(window.event) { 
          // Cancel event handling for <a> tags in IE: 
       //   stopPropagation = true;
//          alert(window.event.srcElement.tagName); 
          // return true if default action is to be performed, false otherwise:
          return((window.event.srcElement.tagName == 'INPUT') || 
...
Aurita.context_menu_click = function(params)
{
    ...
    Aurita.load({ element: 'context_menu', 
                  action: params.url, 
                  onload: function() { Aurita.GUI.center_element('context_menu'); Element.show('context_menu'); } }); 
}; 
...
Aurita.context_menu_over = function(type, args, elem_id, highlight_element_id) 
{
  ...
  if(Aurita.last_hovered) { Element.removeClassName(Aurita.last_hovered, 'context_hover'); }
  if(highlight_element_id) { 
    Element.addClassName(highlight_element_id, 'context_hover'); 
    Aurita.last_hovered = highlight_element_id; 
  }
  else { 
