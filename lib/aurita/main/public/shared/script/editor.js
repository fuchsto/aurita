...
Aurita.Editor = { 
  ...
  {
    if(!$(textarea_element_id)) { log_debug('No such element in init: ' + textarea_element_id); return; }
    ...
    { 
      ...
      log_debug('Registering event handlers for editor ' + textarea_element_id); 
      Event.observe($(textarea_element_id + '_ifr').contentDocument, 'focus', function() { 
          Aurita.form_field_onfocus(textarea_element_id); 
      }); 
      Event.observe($(textarea_element_id + '_ifr').contentDocument, 'blur', function() { 
          Aurita.form_field_onblur(textarea_element_id); 
      }); 
    }
    else { 
      log_debug('Editor '+textarea_element_id+' already in register, skipping flush'); 
    }
  }, 

  save : function(textarea_element_id) 
  {
    if(!$(textarea_element_id)) { log_debug('No such element in save: ' + textarea_element_id); return; }
    if(Aurita.Editor.registered_editors[textarea_element_id] &&
       Aurita.Editor.registered_editors[textarea_element_id] != null) 
    { 
  ...
  //  tinyMCE.execInstanceCommand(textarea_element_id,'mceCleanup');
      ...
      log_debug('Editor '+textarea_element_id+' not in register, skipping save'); 
    }
  }, 

  flush : function(textarea_element_id)
  {
    if(!$(textarea_element_id)) { log_debug('No such element in flush: ' + textarea_element_id); return; }
    if(Aurita.Editor.registered_editors[textarea_element_id] &&
       Aurita.Editor.registered_editors[textarea_element_id] != null) 
    { 
  ...
  //  tinyMCE.execInstanceCommand(textarea_element_id,'mceCleanup');
      ...
      log_debug('Editor '+textarea_element_id+' not in register, skipping flush'); 
    }
  },
...
//    }
  ...
  switch_mode_full : function() { 
    tinyMCE.init({
      // do not provide mode if Editor inits are ALWAYS handled event-based when needed. 
      mode: 'exact', // Do not use textareas or specific_textareas, 
                     // as tinyMCE would kill textarea id's if there 
                     // is more than one editor in a form (Nasty one!)
      editor_selector : "full", 
      plugins : "autoresize,safari,spellchecker,table,iespell,inlinepopups,insertdatetime,fullscreen,visualchars,xhtmlxtras,auritalink,auritafile,link,paste",
      theme : "advanced",
      relative_urls : true,
      valid_elements : "*[*]",
      ...
      theme_advanced_buttons1 : "bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,|,formatselect,removeformat,|,insertdate,inserttime,|,forecolor,backcolor,auritalink,unlink,|,pastetext,pasteword", 
