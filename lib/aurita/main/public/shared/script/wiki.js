...
Aurita.Wiki = { 
  ...
  insert_link : function(article_id_element, url_element) { 
    ...
    if($(article_id_element)) { 
      ...
      url = 'Wiki::Article/show/id='+article_id; 
      var old_c = tinyMCE.activeEditor.selection.getContent(); 
      var new_c = '<a onclick="Aurita.load({ action: \''+url+'\');" href="#'+url+'">'+old_c+'</a>';
      tinyMCE.activeEditor.selection.setContent(new_c); 
      return; 
    }
    else { 
      url = $(url_element).value;
      var old_c = tinyMCE.activeEditor.selection.getContent(); 
      var new_c = '<a href="'+url+'" target="_blank">'+old_c+'</a>';
      tinyMCE.activeEditor.selection.setContent(new_c); 
      return; 
    }
  }, 

  insert_file : function(media_asset_id) { 
    var elm  = '';
//  var type = params['type'];
    var url  = 'Wiki::Media_Asset/show/id='+media_asset_id;
    ...
    if(type == 'image') { 
      ...
      if(tinyMCE.activeEditor.selection) { 
        var old_c = tinyMCE.activeEditor.selection.getContent(); 
        elm = '<a href="#'+url+'" onclick="Aurita.load({ action: \''+url+'\' });">'+old_c+'</a>'
        tinyMCE.activeEditor.selection.setContent(elm); 
      } 
      else { 
        elm = '<a href="#'+url+'" onclick="Aurita.load({ action: \''+url+'\' });">'+params['title']+'</a>'
        tinyMCE.execCommand('mceInsertContent', false, elm);
      }
    }
  }
